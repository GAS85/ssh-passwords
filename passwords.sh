...
# ---------------------------------------------------------------------------
LOG_FILE_BASE="$LOGS_DIR/ssh-auth-logger.log.1"

if [[ -e "$LOG_FILE_BASE" ]]; then
    COMMAND=grep
    LOG_FILE_NAME="$LOG_FILE_BASE"
elif [[ -e "$LOG_FILE_BASE.gz" ]]; then
    COMMAND=zgrep
    LOG_FILE_NAME="$LOG_FILE_BASE.gz"
else
    echo "Log file not found" >&2
    exit 1
fi

$COMMAND -h "Request with password" "$LOG_FILE_NAME" | jq -r '.password | @json' | sort > "$RAW"
